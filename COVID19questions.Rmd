...
h<-ggplot( data = NSW, aes( date, cases )) + geom_point() + theme_bw() + ylab("Confirmed cases in NSW") + scale_x_date(date_minor_breaks=("1 days")) + ylim(0,NA) + 
  ...
  annotate("text",x=as.Date("2020/3/28"),y=2200, label="20% growth daily", color="blue", size=3, angle=70)
show(h)
```
...
mm<-ggplot( data = NSW, aes( date, exponential )) + geom_point(color="blue") + theme_bw() + ylab("Confirmed cases in NSW") + scale_x_date(date_minor_breaks=("1 days")) + ylim(0,NA) + 
  ...
  annotate("text",x=as.Date("2020/3/13"),y=38, label="10% daily growth", color="darkgreen", size=3, angle=15) +
  annotate("text",x=as.Date("2020/3/15"),y=112, label="20% daily growth", color="blue", size=3, angle=28) +
  annotate("text",x=as.Date("2020/3/16"),y=370, label="30% daily growth", color="red", size=3, angle=38)
show(mm) 
```
...
Now use your skills to interpret this plot produced by the Financial Times.
...
Doubling every two days, as the U.S. was doing for about two weeks, corresponds to a `r USrate`% daily growth rate.
...
```{r ideaimage2}
question("'How many purportedly mask-wearing countries is the causal inference based on?",
    answer("1."),
    answer("2."),
    answer("3."),
...
The inference that masks cause less spreading of infection based on only four instances of mask-wearing countries. The fewer the number of instances, the more likely it is that a third variable could explain the difference between the two sets of countries.
...
There are many kinds of differences between countries, from genetic to cultural to environmental and governmental. As a result, the most likely explanation for the lower infection growth rate of the four circles countries is a combination of causes, one of which *may* be masks. The evidence that masks are a cause is weak.

But in an exponentially-growing pandemic, one doesn't have time to wait for strong evidence! One has to do something. Actually, one should do lots of things that may plausibly combat the crisis. Wearing masks is a plausible cause of reduced transmission, so it probably should be adopted.
